...
export class ValidationHelper {
  ...
  static error(message: string, field?: string): ValidationResult {
  ...
  static validateNumber(
    ...
    if (isNaN(numValue)) {
      return this.error(`${fieldName} must be a valid number`, fieldName);
    }

    if (min !== undefined && numValue < min) {
      return this.error(`${fieldName} must be at least ${min}`, fieldName);
    }

    if (max !== undefined && numValue > max) {
      return this.error(`${fieldName} cannot exceed ${max}`, fieldName);
    }
