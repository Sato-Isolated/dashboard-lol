...
export type GetSummonerRequest = z.infer<typeof getSummonerRequestSchema>;
...
export type ChampionMasteryDb = z.infer<typeof championMasteryDbSchema>;

// Validation helper functions
export function validateSummonerName(name: string): {
  isValid: boolean;
  error?: string;
} {
  try {
    summonerNameSchema.parse(name);
    return { isValid: true };
  } catch (error) {
...
export function validateRegionalRegion(region: string): {
  isValid: boolean;
  error?: string;
} {
  try {
